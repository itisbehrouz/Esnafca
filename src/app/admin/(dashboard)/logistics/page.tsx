...
export default function AdminLogisticsPage() {
  ...
  return (
    <div className="space-y-6 font-sans select-none pb-16">
      ...
      {/* Stand-less Merchant Notice (Sleek Inline Action Badge) */}
      {eligibleWithoutStand.length > 0 && (
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 px-3.5 py-2 rounded-xl bg-blue-500/10 border border-blue-500/20 text-xs animate-in fade-in">
          <div className="flex items-center gap-2">
            <ShieldCheck className="w-4 h-4 text-blue-500 shrink-0" />
            <span className="font-bold text-slate-800 dark:text-slate-200">
              {eligibleWithoutStand.length} Doğrulanmış Esnaf Stand Bekliyor
            </span>
            <span className="text-[11px] text-slate-500 dark:text-slate-400 hidden md:inline">
              (Pro/Plus sözleşmeli ücretsiz sayaç pleksi QR kiti)
            </span>
          </div>
          ...
          <button
            ...
            className="px-3 py-1 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold shrink-0 transition-colors shadow-xs cursor-pointer w-fit"
          >
      ...
      {/* Search & Status Filter Chips Bar */}
      <div className="p-3 rounded-2xl bg-white dark:bg-[#0B1120] border border-slate-200 dark:border-slate-800 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex items-center gap-1.5 bg-slate-100 dark:bg-slate-900 p-1 rounded-xl overflow-x-auto scrollbar-none">
          {[
            { id: "all", label: `Tümü (${shipments.length})` },
            { id: "PENDING_PRINT", label: `Baskı Bekleyen (${metrics.pendingPrintCount})` },
            { id: "PRINTING", label: `Baskıda (${metrics.printingCount})` },
            { id: "SHIPPED", label: `Kargoda (${metrics.shippedCount})` },
            { id: "DELIVERED", label: `Teslim Edildi (${metrics.deliveredCount})` },
          ].map((st) => (
            <button
              key={st.id}
              type="button"
