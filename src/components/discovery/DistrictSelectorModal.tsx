...
export function DistrictSelectorModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
      <div 
        className="w-full max-w-md bg-[#F2F2F7] rounded-t-ios-sheet sm:rounded-ios-sheet max-h-[85vh] flex flex-col overflow-hidden shadow-ios-sheet animate-in slide-in-from-bottom sm:zoom-in-95 duration-200"
      ...
      >
        ...
        {/* Modal Header */}
        <div className="px-4 py-3 flex items-center justify-between border-b border-black/[0.06] bg-white/80 backdrop-blur-md">
          {viewState === "districts" ? (
            <button
              onClick={() => setViewState("cities")}
              className="flex items-center gap-1 text-xs font-bold text-brand ios-press py-1 -ml-1 pr-2 rounded-full hover:bg-black/[0.04]"
            >
            ...
            <div className="flex items-center gap-1.5">
              ...
              <h2 className="font-extrabold text-black text-sm">Konum Seç</h2>
            </div>
          )}

          <span className="text-xs font-extrabold text-black truncate max-w-[160px]">
          ...
          <button
            ...
            className="w-7 h-7 rounded-full bg-zinc-200/80 hover:bg-zinc-300 flex items-center justify-center text-zinc-600 ios-press"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="p-3 bg-white border-b border-black/[0.06]">
          <div className="relative flex items-center">
            ...
            <input
              ...
              className="w-full pl-9 pr-8 py-2 rounded-full bg-zinc-100 border border-transparent focus:border-black/[0.08] focus:bg-white text-xs font-medium text-black placeholder:text-zinc-400 outline-none transition-all"
            />
            {searchQuery && (
              <button
                onClick={() => setSearchQuery("")}
                className="absolute right-3 text-xs font-bold text-zinc-400 hover:text-black"
              >
          ...
          {searchQuery.trim() ? (
            <div className="space-y-1.5">
              ...
              {searchResults.length > 0 ? (
                <div className="bg-white rounded-ios border border-black/[0.04] divide-y divide-black/[0.04] shadow-sm overflow-hidden">
                  {searchResults.map((res, i) => (
                    <button
                      key={i}
                      onClick={() => handleNeighborhoodSelect(res.city, res.district, res.neighborhood)}
                      className="w-full px-4 py-3 text-left text-xs hover:bg-zinc-50 flex items-center justify-between ios-press"
                    >
                      <div className="space-y-0.5">
                        <span className="font-bold text-black block">{res.neighborhood}</span>
                ...
                <div className="p-6 text-center text-xs text-zinc-400 bg-white rounded-ios">
            ...
            <div className="space-y-3">
              ...
              <button
                ...
                className={`w-full p-3.5 rounded-ios bg-white border border-black/[0.04] shadow-sm flex items-center justify-between text-xs font-extrabold ios-press ${
                  selectedCity === "Tüm Şehirler" ? "text-brand ring-2 ring-brand/30" : "text-black"
                }`}
              >
                  ...
                  {CITIES.map((city) => {
                    ...
                    return (
                      <button
                        ...
                        className={`p-3.5 rounded-ios text-left bg-white border transition-all shadow-sm flex items-center justify-between ios-press ${
                          isSelected
                            ? "border-brand ring-2 ring-brand/30 text-brand"
                            : "border-black/[0.04] text-black hover:border-zinc-300"
                        }`}
                      >
                        <div className="space-y-0.5">
                        ...
                        <span className="text-[10px] font-extrabold px-1.5 py-0.5 rounded-md bg-zinc-100 text-zinc-600">
                          {city.shortCode}
                        </span>
                      </button>
                    );
                ...
                <div className="space-y-2">
                  {activeCity.districts.map((dist) => {
                    ...
                    return (
                      <div
                        key={dist.name}
                        className="bg-white rounded-ios border border-black/[0.04] shadow-sm overflow-hidden"
                      >
                        {/* District Row */}
                        <div className="p-3.5 flex items-center justify-between hover:bg-zinc-50 transition-colors">
                          <button
                            ...
                            className="flex-1 text-left font-extrabold text-xs text-black ios-press"
                          >
                            <span>{dist.name}</span>
                          ...
                          <button
                            ...
                            className="p-1.5 rounded-full bg-zinc-100 hover:bg-zinc-200 text-zinc-600 text-[11px] font-bold flex items-center gap-1 ios-press ml-2"
                          >
                        ...
                        {isExpanded && (
                          <div className="bg-zinc-50/80 border-t border-black/[0.04] p-2 grid grid-cols-1 gap-1">
        ...
        {/* Footer */}
        <div className="p-3 bg-white/80 border-t border-black/[0.06] text-center">
